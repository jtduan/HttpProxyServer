...
import com.hekewangzi.httpProxyServer.ComputerSide;
...
public class HttpProxyThread extends Thread {
    ...
    public HttpProxyThread(String host, int port) { //手机端构造函数
        super();
        this.clientMode = false;
        try {
            this.phoneSocket = SocketUtil.connectServer(host, port, Properties.ServerConnectTimeout);
            this.clientSocket = phoneSocket;
    ...
    public HttpProxyThread(Socket clientSocket) { //电脑端构造函数
        super();
    ...
    public void run() {
        ...
        try {
            if (this.clientMode) { //电脑端
                this.serverSocket = ComputerSide.computerSockets.get(0);
            } else { //手机端
